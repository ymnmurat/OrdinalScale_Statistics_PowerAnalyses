...
#Let's estimate the power levels and obtain representative figures. Note1: Since there some computational power needed to generate simulations, you may need to wait for a while for the function to execute. Note2: you may be prompted to modify some of the arguments, in case there is some misspelling or missing data. For example, the command below will prompt you with correcting your control, and will take EXP2 and exp3 into account in making the analyses, since there is no group called Experiment1 in our samples, and the function can handle case-sensitivity.
